...
export default function Matches() {
  ...
  useEffect(() => {
    async function fetchProfiles() {
      ...
      try {
        const allProfilesResponse = await client.graphql({
        ...
        if (currentUserProfile && currentUserProfile.skillLevel) {
          ...
          const currentUserSkillIndex = skillTiers.indexOf(
            currentUserProfile.skillLevel
          );
          const friendRequestsResponse = await client.graphql({
          ...
          const friendRequests =
            friendRequestsResponse.data.listFriendships.items;
          const friendProfiles = friendRequests.map((request) => request.id);
          const matchedProfiles = filteredProfiles.filter((profile) => {
            ...
            return (
              profile.behavior === currentUserProfile.behavior &&
              ...
              profile.preferredTeammateAgeRange ===
                currentUserProfile.preferredTeammateAgeRange &&
              profile.preferredGameMode ===
                currentUserProfile.preferredGameMode &&
              profile.preferredRole === currentUserProfile.preferredRole &&
              profile.flexibility === currentUserProfile.flexibility &&
              profile.communicationPreference ===
                currentUserProfile.communicationPreference &&
              ...
              profile.competitivenessLevel ===
                currentUserProfile.competitivenessLevel
            );
          });
          ...
          const nonFriendProfiles = matchedProfiles.map((profile) => {
            const friendRequest = friendRequests.find(
              (request) =>
                request.id === profile.id || request.id === profile.id
            );
            const status = friendRequest?.friendshipStatus;
  ...
  return (
    <div className="mb-6 flex flex-wrap">
      ...
      {matchedProfiles.map((profile) => (
        <div
          className="w-full max-w-sm border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mx-2 mb-4"
          key={profile.id}
        >
